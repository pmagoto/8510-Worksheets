...
avg.city.in.SC <- gayguides %>%
    select(state, city, Year, title) %>%
      filter(state == southernstates & Year >= 1970 & Year <= 1975) %>%
        group_by(city) %>%
          summarise(SC.loc.numbers = n()) %>% 
            summarise(mean(SC.loc.numbers))
...
#this would be above, except replace state = SC with this list... 
```
...
wide.sc.parks <- sc.parks %>% 
  pivot_wider(names_from = year, values_from = type_of_worker)
