...
```{r}
new.ac.lda <- LDA(ac.dtm, k = 2, control = list(seed = 12345))

new.ac.topics <- tidy(new.ac.lda, matrix = "beta")
head(new.ac.topics)

new.ac.top.terms <- new.ac.topics %>%
  arrange(desc(beta)) %>% 
  filter(topic == 1) %>% 
  slice(1:10)
...
```{r}
# Metadata that includes info about each issue.
mb.metadata <- read.csv("https://raw.githubusercontent.com/regan008/8510-TextAnalysisData/main/mb-metadata.csv") %>%  mutate(Filename = paste0("MB_", Filename))
library(tidyverse)
```
...
```{r}
numbers <- c(1:100)
filepaths.mb <- system.file("mbtxt/")
mb.txts <- readtext(paste("mbtxt/", "*.txt", sep=""))
mb.whole <- full_join(mb.metadata, mb.txts, by = c("Filename" = "doc_id")) %>% as_tibble() 
tidy.mb <- mb.whole %>%
  unnest_tokens(word, text) %>% 
  filter(str_detect(word, "[a-z']$")) %>% 
  anti_join(stop_words) %>% mutate()
head(tidy.mb)
mb.counts <- tidy.mb %>% 
  count(Filename, word) %>% 
  cast_dtm(Filename, word, n)
...
stop_words_custom <- stop_words %>% 
  add_row(word="volume", lexicon="NA") %>% 
  add_row(word="june", lexicon="NA") %>%
  add_row(word="july", lexicon="NA") %>% 
  add_row(word="august", lexicon="NA") %>% 
  add_row(word="september", lexicon="NA") %>% 
  add_row(word="september", lexicon="NA") %>% 
  add_row(word="october", lexicon="NA") %>% 
  add_row(word="november", lexicon="NA") %>% 
  add_row(word="december", lexicon="NA") %>%
  add_row(word="january", lexicon="NA") %>% 
  add_row(word="february", lexicon="NA") %>%
  add_row(word="march", lexicon="NA") %>% 
  add_row(word="april", lexicon="NA") %>% 
  add_row(word="may", lexicon="NA") %>% 
  add_row(word="volume", lexicon="NA") %>% 
  add_row(word="page", lexicon="NA") %>% 
  add_row(word="vol", lexicon="NA") %>% 
  add_row(word="th", lexicon="NA") 
new.tidy.mb <- mb.whole %>%
  unnest_tokens(word, text) %>% 
  filter(str_detect(word, "[a-z']$")) %>% 
  anti_join(stop_words_custom)
tidy.mb.words <- new.tidy.mb %>% count(Filename, word)
new.mb.counts <- tidy.mb.words %>% 
  count(Filename, word) %>% 
  cast_dtm(Filename, word, n)
head(new.mb.counts)
mb.lda <- LDA(new.mb.counts, k = 5, control = list(seed = 12345))
...
```{r}
mb.1920s <- new.tidy.mb %>% filter(Year >= 1920 & Year <= 1929)
mb.1920s.count<- mb.1920s %>% 
  count(Filename, word) %>% 
  cast_dtm(Filename, word, n)
mblda.1920 <- LDA(mb.1920s.count, k = 5, control = list(seed = 12345))
mb.1920topics <- tidy(mblda.1920, matrix = "beta")
mb.1920.top.terms <- mb.1920topics%>%
  arrange(desc(beta)) %>% 
  group_by(topic) %>% slice(1:5)
mb.1920.top.terms %>%
  mutate(term = reorder_within(term, beta, topic)) %>%
...
#I really should have take "health" out with my stop words.... I did not, but R kept crashing im sorry. 
...
library(tidyverse)
library(lubridate)
bb.metadata.correct <- bb.metadata %>% 
  mutate(Date = mdy(Date), day = day(Date), month = month(Date, label = TRUE), year = year(Date))
...
fullbbdata <- inner_join(correct.bb.metadata, bb, by = "doc_id") %>% 
  as_tibble()
bb.stopwords <- stop_words %>% 
  ...
  add_row(word="west", lexicon="NA") %>% 
  add_row(word="wild", lexicon="NA") %>% 
  add_row(word="bill's", lexicon="NA")
tidybb <- fullbbdata %>% 
  unnest_tokens(word, text) %>%
  anti_join(bb.stopwords) %>%
  group_by(doc_id, Date, Publication, year) %>%
  count(word, sort = TRUE)
tdy.bb.words <- tidybb %>% count(doc_id, word)
bb.counts <- tdy.bb.words %>% 
  count(doc_id, word) %>% 
  cast_dtm(doc_id, word, n)
bb.lda <- LDA(bb.counts, k = 5, control = list(seed = 12345))
...
```
```{r}
#what difference does 20 years have on the topics being shown here?

tidy1883words <- tidybb %>% filter( year == 1883) %>% count(doc_id, word)
tidy1883counts <- tidy1883words %>% 
  count(doc_id, word) %>% 
  cast_dtm(doc_id, word, n)
lda1883<- LDA(tidy1883counts, k = 6, control = list(seed = 12345))
topics1883 <- tidy(lda1883, matrix = "beta")
topterms1883 <- topics1883 %>%
  arrange(desc(beta)) %>% 
...
```{r}
tidy1903words <- tidybb %>% filter( year == 1903) %>% count(doc_id, word)
tidy1903counts <- tidy1903words %>% 
  count(doc_id, word) %>% 
  cast_dtm(doc_id, word, n)
lda1903<- LDA(tidy1903counts, k = 6, control = list(seed = 12345))
topics1903 <- tidy(lda1903, matrix = "beta")
topterms1903 <- topics1903 %>%
  arrange(desc(beta)) %>% 
  group_by(topic) %>% slice(1:10)
topterms1903 %>%
  mutate(term = reorder_within(term, beta, topic)) %>%
...
I wanted to discern co-occurrences that differed between 1883 and 1903, trying to see if the topics changed as much over 20 years as they could. What I tended to notice is that in 1803 they are very Midwest centered with their topics and the 1903 topics expand their scope to more western states and even into Europe. Something that was obvious throughout was the use of the words "Indian", "camp" and "fair".
Interestingly, the 4th, 5th, and 6th topics that is generated in 1903 is heavily political, and I would love to see what change happened here in regards to the Buffalo Bill news articles. Why are we so often discussing Europe in relationship to private organizations or the news?

Is the first topic in the 1903 topic model visualization in regards to when Annie Oakley got arrested for stealing in Chicago to support her cocaine habit? I know this is a long shot but, annie, oakley, law ans chicago is the onlu incident I can think of when Annie Oakley ever got arrested. 
