...
```{r}
paste("My name is", "Paige Magoto")
```

(@) Can you explain what happened in 2-3 sentences? 

> In the code box above, I used the fuction "paste()" to put twostirngs together. By putting these two strings, "My name is" and "Paige Magoto" together they show up in the console as 1 singuar output. The Singualr output was the full sentence "My name is Paige Magoto".
...
> The 6 Lines above are logical values, they are comparing number inputs to oneanther to produce a TRUE or FALSE output.
95= FALSE
96= TRUE
97= TRUE
98= FALSE
...
>The variable my.numbers is actually a vector because it has more than one item, it contains 4 numbers. By multiplyin ght evector my.numbers by 2, each number tht is contained uner my.numbers is multiplied by two and the output is each number doubled. 
...
> Above, the only number being multiplied by 2 is the 3rd number in th elist under the vector my.numbers. Therefore, 6 is the number in ht ethird position under my.numbers, so 6 is th eonly number being mulitplied by 2. The breackets [] determine whihc number/item in the vector is being targeted. 

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
pages.read <- c(22, 65, 12, 8, 45)
pages.read
```
...
>Row Z is multiplying row x by 2 and then adding y. 
...
>Originally it says "Bob Damron's Address Books" and then once clicked into it it says: "This dataset contains information transcribed from Bob Damron's Address Books from 1965 through 1985. Last updated December 2022."
...
>For the first 10 rows of the data set, I googled how to modify the head() funtion, and noticed you could modify it to any length of data. For the tail() fucntion I googled how to show the last 6 rows of a dataset in R.
...
> "gayguides" is the datat set, R is using this name to pull up the data to present. The $ tells R which segent of the data to pull up, so when paired with "title" the only datat thT is displayed from the datat set titile category. The [500] is referncing which line of the dataset that needs to be pulled up. the brackets help the user sort through large data sets by isolating one line of data and presenting it. 
...
> The most prominent thing I noticed from this dataset was using the tail() function, where I realized there were 10,702 women listed in this data set. 
