...
> According to the metadata, some of the stou addresses are written and some are spoken. Its possibel that people wre writing longer addresses rather than those that will be spoken and preformed in front of an audience. 

(@) Filter the dataset to address this discrepancy and the recreate these statistics: 
```{r}
sotu_year <- tidy_sotu_words %>% group_by(year)%>% summarise(count = n())
sotu_speech <- tidy_sotu_words %>% group_by(year)%>% filter(sotu_type == "speech")  %>% summarise(speech = n())
sotu_written <- tidy_sotu_words %>% group_by(year) %>% filter(sotu_type == "written") %>% summarise(written = n())
sotu_all <- left_join(sotu_speech, sotu_written, by = "year") 
sotu_longer <- pivot_longer(sotu_all, !year, names_to = "type.of.speech", values_to = "count")
ggplot(data = sotu_longer, mapping = aes(x = year, y= count, group = type.of.speech, color = type.of.speech)) + geom_line() + labs( x = "Year", y= "Number of Words per State of the Union", title = "Number of Words in state of the Union over time by type of Address", color = "Type of Adress") + theme(plot.title = element_text(hjust = 0.5)) + theme_dark() + geom_point()
```
> above shows the idfferntce, while displaying all the data
...
> tidy_sotu %>%
  count(president, word)  %>%  # counting each word in the parameters of of the presidents
  group_by(president) %>% 
  mutate(n_tot = sum(n), # creating a new column 
         term_freq = n/n_tot) %>% # this new column shows the frewuency of the word used grouped by president 
  arrange(desc(term_freq)) %>% # sort by term frequency
  top_n(1) %>%  # take the top for each president, shows the most ued wor by each president
  print(n = Inf) # print all rows
...
```{r}
jimmycarter.sotu <- tidy_sotu %>% filter(president == "Jimmy Carter") %>% group_by(president, word) %>% count(president, word, sort = TRUE)

jimmycarter.tfidf <- tidy_sotu %>% filter(president == "Jimmy Carter") %>%  group_by(doc_id, word) %>% count(doc_id, word, sort = TRUE) %>% bind_tf_idf(word, doc_id, n)

jimmycarter <- merge(jimmycarter.sotu, jimmycarter.tfidf, by = "word")

#Jimmy carter's most used word by count is congress in all of his speeches, but the word used the most in one singular speech is actually afghanistan. 
...
bb.metadata.correct <- bb.metadata %>% mutate(Date = mdy(Date), day = day(Date), month = month(Date, label = TRUE), year = year(Date))
```
...
```{r}
correct.bb.metadata <- bb.metadata.correct %>% rename(doc_id = Filename)
fullbbdata <- inner_join(correct.bb.metadata, bb, by = "doc_id") %>% as_tibble()
bb.stopwords <- stop_words %>% add_row(word="Buffalo", lexicon="NA") %>% add_row(word="Bill", lexicon="NA") %>% add_row(word="Cody", lexicon="NA") %>% add_row(word="bill", lexicon="NA") %>% add_row(word="buffalo", lexicon="NA") %>% add_row(word="cody", lexicon="NA")
tidybb <- fullbbdata %>% unnest_tokens(word, text) %>% anti_join(bb.stopwords) %>% group_by(doc_id, Date, Publication, year) %>% count(word, sort = TRUE)
bb.tfidf <- tidybb %>% bind_tf_idf(word, doc_id, n)
```
...
```{r}
#1896 had the most unique words in the corpus, so i decided to research this one longer. 
bb1896 <- bb.tfidf %>% filter(year == 1896)

```

```{r}
#comparing the use of city and wild through the years
bb.tfidf %>%
  filter(word %in% c("city", "wild")) %>% 
  count(year, word) %>% 
  ggplot(aes(year, n, fill = word)) +
    geom_col(position = "fill")
```
...
```{r}
#use of the word city and indian at the same time
bb.tfidf %>%
  filter(word %in% c("indian", "city")) %>% 
  count(year, word) %>% 
  ggplot(aes(year, n, fill = word)) +
    geom_col(position = "fill")
```
...
```{r}
indian <- tidybb %>% group_by(year) %>% filter(word == "indian")  %>% summarise(count.indian = n())
wild <- tidybb %>% group_by(year) %>% filter(word == "wild") %>% summarise(count.wild = n())
indian.wild <- merge(indian, wild, by = "year")
longer.indian.wild <- pivot_longer(indian.wild, !year, names_to = "word", values_to = "count")
ggplot(data = longer.indian.wild, mapping = aes(x = year, y= count, group = word, color = word)) + geom_line() + labs( x = "Year", y= "Use of Words", title = "Use of the Words Indian and Wild over time", color = "Word") + theme(plot.title = element_text(hjust = 0.5)) + theme_dark() + geom_point()
```


(@) Finally, when you are all done. Write up your findings. What research question did you focus on and what did you learn? 

I explored the use of the words "indian" and "wild" throughout the years. With this final visualization, you can see that there is a positive correlation between the use of "Indian" and "Wild", and you can conclude they were often used within the same context using this visualization. This could be offset by the use of the word west, as often times wild west is a term used in this type of literature. The Graph below concludes that yes, wild and west are OFTEN used together, but they are still associated with indian.

```{r}
west <- wild <- tidybb %>% group_by(year) %>% filter(word == "west") %>% summarise(count.wild = n())
indian.wild.west <- merge(west, indian.wild, by ="year")
longer.indian.wild.west <- pivot_longer(indian.wild.west, !year, names_to = "word", values_to = "count")
ggplot(data = longer.indian.wild.west, mapping = aes(x = year, y= count, group = word, color = word)) + geom_line() + labs( x = "Year", y= "Use of Words", title = "Use of the Words Indian, Wild, and West over time", color = "Word") + theme(plot.title = element_text(hjust = 0.5)) + theme_dark() + geom_point()
```
