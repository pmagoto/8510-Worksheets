...
>Above, by adding a negative integer in the function you are excluding those two numbers in the subset. So, with this above, you are only showing the numbers in the 2nd, 4th-7th position and excluding the 1st and 3rd. 
...
>The TRUE?FALSE vector is only printing the values that are being labeled as true, so the first, third and fourth value of the original x list. The next is only printing the integers that are greater than seven. 
...
> Above, the vector is being named y, but the only integers within this vector is the first 4 numbers, they are then being set to letters as their names. a, b, c, and d are being set, but the last foru letters of the integers are not being given name values. if the code above reflected 1:7 instead of 1:4 the integers would be named a, b, c, d, e, f, g. Then, they print the integer named "d", which is 8.35.
...
> The first subset is showing ONLY the second and third columns of the data set. The second subset, is showing the first three rows, in their entirety. I believe this is intitiated by the comma the end of the integer pair. 
...
BostonWomenVoters[1:5,][7:11]

```

> I wanted to just diplay the name and age of the first 5 rows of data, but I couldnt figure out how to disclude the 8th, 9th, and 10th, rows of data. I was thinking this
BostonWomenVoters[1:5,][BostonWomenVoters$Name, BostonWomenVoters$Age]
...
> In the "City" column, this code is gving the data for the 100th-110th row. only outputting the column that is designated city. 
...
students <- 22
if (students >= 30) {
  print("Student cap met.")
} else if (students < 30){
  print("Student cap not met.")
}
```
...
> I checked the range of the Year column of the datat set, sicne it was an integer, I know tht the highest date is 1985. 
I tried it this way first, bu it looped over and over so I assume that wasnt right. 
for (y in 1:length(gayguides$Year)){
+     if (p >= 1990){
+         print(p)
+     } else if (p < 1990){
+         print("There are not places during/after 1990")
+         break
+     }
+ }
...
> length shuffles through the whole list in this loops, generally it will tell how long a list/data set is in columns. 
...
```{r}
classes <- c("Digital Methods 2", "American Research Seminar", "History of Sex Work")
...
> the loop automatically skipped over 1 and 2 because the next statement said it had to be over 3, and then the break statement said that the loop would terminate after 5 or greater. 
...
range(statepopulations$X1800, na.rm = TRUE)
...
```{r}
historyStudents("paige", "HIST 8510")
```
...
```{r}
Letters_in_my_name <- nchar("Paige Magoto")
print(paste("there are", Letters_in_my_name, "characters in that string"))
...
>Because you arent usuing the function correctly, you should write is as i have chnaged above. You have to print george washington as the grade is assignmened to him. 


Here's a more complex example: 
```{r}
gg.locations <- function (city, state, year) {
  data(gayguides)

  for (i in 1:length(gayguides$city)) {
    if (gayguides$city[i] == city && gayguides$state[i] == state && gayguides$Year[i] == year) {
...
gg.locations("Greenville", "SC",  1985)
```
...
bwv.stateandoccupation <- function(State.or.Province.of.Birth, occupation, age){
  data("BostonWomenVoters")
  
  for (n in 1:length(BostonWomenVoters$State.or.Province.of.Birth)){ 
    if (BostonWomenVoters$State.or.Province.of.Birth[n] == State.or.Province.of.Birth && BostonWomenVoters$Occupation[n] == occupation && BostonWomenVoters$Age[n] == age){
      print(paste(BostonWomenVoters$Name[n], "and she is a", occupation, "she was born in", State.or.Province.of.Birth,".", "She is", age, "years old."))
    }
    }
}

bwv.stateandoccupation ("Massachusetts" , "Tel. operator", 23 )
bwv.stateandoccupation ("New Jersey" , "Widow", 50)
bwv.stateandoccupation ("Pennsylvania" , "Housewife", 39 )
#how would i get it do both iterations of housewife? 
