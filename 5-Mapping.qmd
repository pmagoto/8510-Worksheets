...
streetaddresses.BWV <- BostonWomenVoters %>% mutate(BostonWomenVoters, streetaddress = paste0(Street.Number.on.April.1, " ", Street.of.Residence.on.April.1, ",", " ", Town.of.Birth, ",", " ", State.or.Province.of.Birth)) %>% sample_n(100)
head(streetaddresses.BWV)

BWV.geocoded <- streetaddresses.BWV %>% geocode(address = streetaddress, method='osm', lat = latitude, long = longitude)
head(BWV.geocoded)
```
...
> This dataset includes every US state divided intro groups and areas with latitude and longitude included within them. This is a map essentially. 
...
```{r}
data(gayguides)
gayguides.minushawaii <- gayguides %>% filter(Year == 1965, state != "HI")
ggplot() + 
  geom_map( data = usa, map = usa, aes(long, lat, map_id=region), color = "grey", fill= "antiquewhite") +
  geom_point(data = gayguides.minushawaii, mapping = aes(x=lon, y=lat), color = "blue") + labs(x = "Longitude", y = "Latitude", title = "Gayguides points in 1965")+ theme(plot.title = element_text(hjust = 0.5))
```

(@) Can you map just locations in South Carolina (on a basemap of SC)? 
```{r}
gg.sc <- gayguides %>%  filter(state == "SC")
sc.map <- usa %>% filter(region == "south carolina")
ggplot() + geom_map( data = sc.map, map = sc.map, aes(long, lat, map_id=region),color = "black", fill= "grey") + geom_point(data= gg.sc, mapping = aes(x=lon, y=lat), color = "pink") + labs(x = "Longitude", y = "Latitude", title = "Gayguides points in SC")+ theme(plot.title = element_text(hjust = 0.5))
...
> When you include the HI and AK data points they distort the map, and nearly turn it sideways. A way to fix this could be to facet the data, and make HI and AK their own mini map. 
...
```{r}
total_male_pop <- nhgis_data %>% mutate(malepopulation = AZF001 + AZF003)
```

(@) Now, use the total_male_pop variable and create a variable for the the percentage of foreign born males.
```{r}
percentmforeignborn<- total_male_pop %>% mutate(percentmaleforeignborn = (AZF003 / malepopulation) * 100)
```
...
> scale_fill_continuous() creates a gradient that is not reliant on a set number of colors. this allows the graph to be more readable. type will allow the code writer to choose the color for the visualzation. 

Now create the map: 
```{r}
ggplot(data = percentmforeignborn, aes(fill = percentmaleforeignborn)) +
  scale_fill_continuous(name = "percent of malesforign born", type = "viridis", labels = scales::percent)
```
### Leaflet
...
```{r}
closegg <- c("GA", "SC", "NC", "TN")
southerngg <- gayguides %>% filter(state == closegg) %>%  filter(type == "Bars/Clubs")

leaflet(data=southerngg) %>% addTiles() %>% addMarkers(~lon, ~lat, popup = paste0("This location was recorded in", southerngg$Year , "."))
```
...
> Leaflet is more desireable when you want to provide extra information about the point on the map. Is there a way to integrate ggplot and leaflet? coul the popup be a graph or text analysis? 
...
> Parks in US by size excluding alaska and hawaii bc the map ugly so ill make anotehr one

```{r}
parksUS <- parks %>% filter(country == "United States") %>% filter(state_region_province != "Alaska") %>% filter(state_region_province != "Hawaii")

ggplot() + 
  geom_map( data = usa, map = usa, aes(long, lat, map_id=region), fill="white", color="gray") +
  geom_point(data = parksUS, mapping = aes(x=lon, y=lat, size=total_sq_kilometers, color = total_sq_kilometers)) + labs(x = "Longitude", y = "Latitude", title = "US Parks by Size")
...
```{r}
leaflet(data=parksUS) %>% addTiles() %>% addMarkers(~lon, ~lat, popup = paste0("This park's name is", " ", parksUS$park, "."," ", "This location was created in"," ", parksUS$year ," ", "and is", " ", parksUS$total_sq_kilometers," ", "square kilometers."))
...
```{r}
leaflet(data=parks) %>% addTiles() %>% addMarkers(~lon, ~lat, popup = paste0("This park's name is", " ", parks$park, "."," ", "This location was created in"," ", parks$year ," ", "and is", " ", parks$total_sq_kilometers," ", "square kilometers."))

```



> bar graph for the ammount of parks in each country 

```{r}
parksbycount <- parks %>% group_by(country) %>% select(park, country) %>% summarise(count = n())
ggplot(data = parksbycount, mapping = aes(x = country, y = count)) + geom_col(color = "antiquewhite3", fill = "antiquewhite3") +  geom_text(aes(label = count), vjust = -0.5, color = "black", size = 2) + labs(x = "Country", y = "Number of Parks", title = "Number of Parks by Country") +  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + theme(plot.title = element_text(hjust = 0.5)) + theme(plot.title = element_text(hjust = 0.5))

```

>Ukraine parks over time

```{r}
parksovertimeUkraine <- parks %>% filter(country == "Ukraine") %>% select(year, park, country) %>%  group_by(year) %>% summarise(count =n())

ggplot(parksovertimeUkraine, aes(x = year, y = count)) + geom_line(color = "gray") + geom_point(color = "blue") + geom_text(aes(label = count), vjust = -.65, hjust = -.5, size = 3, color = "blue") + labs(x= "Year", y= "Number of Parks", title = "Number of Parks in Ukraine being added over Time") + theme(plot.title = element_text(hjust = 0.5)) + theme_classic()
...
```{r}
parksovertimeUS <- parks %>% filter(country == "United States") %>% select(year, park, country) %>%  group_by(year) %>% summarise(count =n())

ggplot(parksovertimeUS, aes(x = year, y = count)) + geom_line(color = "gray") + geom_point(color = "blue") + geom_text(aes(label = count), vjust = -.65, hjust = -.5, size = 3, color = "blue") + labs(x= "Year", y= "Number of Parks", title = "Number of Parks in US being added over Time") + theme(plot.title = element_text(hjust = 0.5)) + theme_classic() 

```

how do you increase the x axis increments. 